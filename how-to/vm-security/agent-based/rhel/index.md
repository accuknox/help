---
title: VM Onboarding for RHEL
description: Prepare RHEL hosts and onboard a VM fleet to AccuKnox using the SSH-based fleet onboarding script.
---

# VM Onboarding for RHEL

RHEL host preparation, fleet onboarding, and scanner configuration.
...
If bpf is not in the list, enable BPF LSM, then reboot and recheck. Follow the KubeArmor guide for the kernel boot parameter.
...
Verify passwordless access from the control plane to every host.
...
```bash
sudo firewall-cmd --permanent --add-port=32768-32771/tcp
...
Verify an inbound port from a worker. The expected reply confirms the port is reachable.

```bash
curl <control-plane-ip>:32770
# expected: curl: (1) Received HTTP/0.9 when not allowed
```

### 3.2 Outbound, from the Control Plane

The control plane must be able to establish outbound TCP connections to the AccuKnox services. On most RHEL installations, outbound traffic is allowed by default and no firewall changes are required.

If your organization enforces outbound firewall restrictions, configure your firewall to allow TCP egress to the following endpoints:
...
Verify outbound connectivity to each endpoint and port.
...
Access the script from here: [vm_onboarding.sh](https://gist.github.com/VanishaParwal/4814897c869f4225eb29ba548d5a9fca). Collect four inputs before editing the script: the inventory file, the join token, the artifact label, and the scanner API token.
...
# 2. firewall (firewalld): open 32768-32771 inbound on the control plane, allow egress if your org restricts it by default
