...
### **Use Case 3: Applying Policies for a PyTorch Application**

This guide demonstrates creating, containerizing, and deploying a PyTorch-based application using Kubernetes, while applying and verifying security policies.

#### 1. **Create the PyTorch Application (`app.py`)**

The application trains a simple neural network:
...
for epoch in range(100000):  # 100000 epochs
    optimizer.zero_grad()
...
Using a `Dockerfile` to package the application:
...
The deployment uses Kubernetes CronJob to schedule regular training runs.

**Deployment YAML (`pytorch-deployment.yaml`):**

```yaml
apiVersion: batch/v1
kind: CronJob
metadata:
  name: pytorch-cronjob
  namespace: default
  labels:
    app: pytorch-cronjob
spec:
...
Result:
 ![Pods List](./images/modelarmor/3.png)

#### 4. **Verify Policies**

Check the application within the deployed container:

```bash
kubectl exec -it -n default pytorch-cronjob-28649565-tbfmv -- bash
```

Output:
 ![Policy Check](./images/modelarmor/4.png)

#### Summary

This approach demonstrates:

1. Creating a PyTorch app and containerizing it.
2. Deploying it using Kubernetes CronJob.
3. Applying security policies and verifying their effectiveness.

The images illustrate pod monitoring and policy enforcement.

---

### **Use Case 4: Deploying a PyTorch Application with KubeArmor**
