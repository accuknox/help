...
![AccuKnox Release Notes v3.6, with the twelve headline changes grouped into findings, cloud, policy, platform, host, and AI](./images/release-notes/v3.6/v3-6-release-highlights.png)
...
The EC2 instance above is the attack path in one picture. An **Internet** node connects straight to it, and from the instance the edges run out to its VPC, subnet, security group, and the IAM identities attached to it. An attacker who reaches the instance from the internet inherits those identities and can pivot along those edges. Reading that from a list of findings takes a lot longer.
