...
This guide walks you through integrating KICS (Keeping Infrastructure as Code Secure) scans with AccuKnox using a CI/CD pipeline workflow. The scan results, formatted in SARIF, will be enriched with metadata and code snippets and then integrated with AccuKnox for further analysis.

**Pre-requisites**

- GitHub repository with the appropriate permissions.

- Checkmarx KICS API credentials (Client ID, Client Secret, Tenant ID).

- AccuKnox CSPM credentials (Tenant ID, CSPM URL, Token).

## **Step 1: Obtain Checkmarx and AccuKnox Credentials**

### **Getting Checkmarx KICS Credentials**
...
1. **Generate the AccuKnox Token**
    Open AccuKnox and go to **Settings > Tokens**. Click **Create**, give your token a name, and click **Generate**. Once generated, click **Copy** to save the token. Also, note down the Tenant ID.
    For details, refer to [How to Create Tokens](https://help.accuknox.com/how-to/how-to-create-tokens/?h=token "https://help.accuknox.com/how-to/how-to-create-tokens/?h=token").
...
In your GitHub repository, create a workflow file (e.g., `.github/workflows/checkmarx-kics-accuknox.yml`) with the following content:

{% raw %}

```yaml
name: Checkmarx Sarif Integration for KICS
...
![image-20241203-045123.png](./images/checkmarx-iac/1.png)

## **Step 4: Running the Workflow**

1. Push changes to the repository's main branch or open a pull request.

2. The Checkmarx KICS scan will run and generate a SARIF file.

3. The workflow will enrich the SARIF file with metadata and code snippets, and then upload it to AccuKnox for analysis.

![image-20241205-024738.png](./images/checkmarx-iac/2.png)

## **Step 5: View Findings in AccuKnox**

To see your findings navigate to AccuKnox > Issues > Findings and select the **CX KICS** findings.

![image-20241205-024833.png](./images/checkmarx-iac/3.png)

Click on any finding to get more details. You can also click on **Create Ticket** to create a ticket for further resolution.

![image-20241205-025413.png](./images/checkmarx-iac/4.png)

## **Conclusion**

Following this guide, you successfully integrated KICS scans into your CI/CD pipeline and connected them with AccuKnox. This enables you to analyze and prioritize vulnerabilities directly within the AccuKnox dashboard.
